...
router.get("/visits", async (req, res) => {
  ...
  if (year) {
    query = query.where(sql`EXTRACT(YEAR FROM ${visitsTable.visitDate}) = ${year}`);
  }
  if (country) {
    query = query.where(eq(visitsTable.country, country));
  }
...
router.patch("/visits/:id", async (req, res) => {
  ...
  const [visit] = await db
    .update(visitsTable)
    .set(bodyParsed.data)
    .where(eq(visitsTable.id, paramsParsed.data.id))
    .returning();
